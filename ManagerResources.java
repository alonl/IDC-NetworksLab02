...
public class ManagerResources {
    ...
    public ModelAppResponse handleRequest(HTTPRequest request) {
        try {
            if (validateInServerDir(request)) {
                ...
                return ModelAppResponse.response(HTTPConstants.STATUS_BAD_REQUEST, "Don't be evil! Requested page path is outside the server's scope.");
            }
        } catch (IOException e) {
    ...
    private boolean validateInServerDir(HTTPRequest request) throws IOException {
        if (request.getRequestedPage() != null &&
                !request.getRequestedPage().getCanonicalPath().startsWith(request.getServerRoot().getCanonicalPath())) {
            return false;
        }
        return true;
    }
