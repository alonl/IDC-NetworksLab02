...
public class MainRouter {
	...
	public static final String[] PARAMETRIZED_PATHS_ARR = { "submit_reminder.html", "submit_task.html",
			"submit_poll.html", "task_reply.html", "poll_reply.html" };
	...
	public static final String[] REST_REQUESTS_ARR = { HelperConstants.TASKS_PATH, HelperConstants.REMINDERS_PATH,
			HelperConstants.POLLS_PATH };
	private static final HashSet<String> REST_REQUESTS = new HashSet<>(Arrays.asList(REST_REQUESTS_ARR));
	public static final HTTPMethod[] REST_METHODS_ARR = { HTTPMethod.GET, HTTPMethod.HEAD, HTTPMethod.POST,
			HTTPMethod.PUT, HTTPMethod.DELETE };
	private static final HashSet<HTTPMethod> REST_METHODS = new HashSet<>(Arrays.asList(REST_METHODS_ARR));
	public static final HTTPMethod[] STATIC_METHODS_ARR = { HTTPMethod.GET, HTTPMethod.POST, HTTPMethod.PUT,
	...
	public MainRouter(Map<String, ServiceAbstract<? extends ModelBaseItem>> services) {
		...
		this.remindersManager = new ManagerReminders(
				(ServiceAbstractScheduled<ModelReminder>) services.get(ModelReminder.class.getName()));
		this.tasksManager = new ManagerTasks((ServiceAbstractScheduled<ModelTask>) services.get(ModelTask.class
				.getName()));
	}
	...
	private boolean isExcludedSecurity(HTTPRequest request) {
		return request.isResource()
				|| (request.getRequestedPage() != null && EXCLUDED_SECURITY.contains(request.getRequestedPage()
						.getName()));
	}

	private boolean isLoginRequest(HTTPRequest request) {
		return request.getRequestedPage() != null && request.getRequestedPage().exists()
	...
	private ModelAppResponse routeREST(HTTPRequest request, ModelUser user, BufferedReader is) {
		...
		if (path.startsWith(remindersManager.getServicePath())) {
			return remindersManager.handleRequest(method, path.substring(remindersManager.getServicePath().length()),
					body, user);
		} else if (path.startsWith(tasksManager.getServicePath())) {
			return tasksManager.handleRequest(method, path.substring(tasksManager.getServicePath().length()), body,
					user);
		} else if (path.startsWith(pollsManager.getServicePath())) {
			return pollsManager.handleRequest(method, path.substring(pollsManager.getServicePath().length()), body,
					user);
		} else {
			return ModelAppResponse.responseError(new WebServerBadRequestException("Unknown request: " + method + " , "
					+ path));
		}
	}
