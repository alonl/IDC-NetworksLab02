...
public class HelperLogger {
	...
	public void debug(String message) {
		if (logLevel.ordinal() <= LogLevel.DEBUG.ordinal())
			outFormat("DEBUG", loggerName, message);
	}

	public void info(String message) {
		if (logLevel.ordinal() <= LogLevel.INFO.ordinal())
			outFormat("INFO", loggerName, message);
	}

	public void warn(String message) {
		if (logLevel.ordinal() <= LogLevel.WARN.ordinal())
			outFormat("WARN", loggerName, message);
	}

	public void error(String message) {
		if (logLevel.ordinal() <= LogLevel.ERROR.ordinal())
			outFormat("ERROR", loggerName, message);
	}
	...
	public void outFormat(String messageType, String className, String message) {
		out(String.format("%-23s %-5s %-18s: %s", new Timestamp(new Date().getTime()).toString(), messageType,
				className, message));
	}
