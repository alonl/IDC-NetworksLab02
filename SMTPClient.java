...
public class SMTPClient {
	...
	public boolean sendMessage(ModelMailMessage mailMessage) {
		...
		if (sentOk) {
			logger.info(String.format("Mail message from: '%s', to: '%s' sent OK.", mailMessage.getFrom(),
					mailMessage.getTo()));
		} else {
			logger.error(String.format("Error while sending mail message from: %s, to: %s", mailMessage.getFrom(),
					mailMessage.getTo()));
		}
		return sentOk;
	}
